...
template<class T>
_chapter5_arrayList<T> ::_chapter5_arrayList(const _chapter5_arrayList<T>& theList)
{
...
template<class T>
T& _chapter5_arrayList<T>::get(int theIndex) const
{
...
template<class T>
int _chapter5_arrayList<T>::indexOf(const T& theElement) const
{
...
template<class T>
void _chapter5_arrayList<T>::erase(int theIndex)
{
	checkIndex(theIndex);
...
template<class T>
void _chapter5_arrayList<T>::changeLength1D(T*& pa, int oldLength, int newLength)
{
	if (newLength < 0)
...
template<class T>
void _chapter5_arrayList<T>::insert(int theIndex, const T& theElement)
{
...
template<class T>
void _chapter5_arrayList<T>::output(std::ostream& out) const
{
...
std::ostream operator << (std::ostream out, const _chapter5_arrayList<T>& x)
{
	// 输入参数为流对象和线性表实例的引
...
template<class T>
void _chapter5_arrayList<T>::checkIndex(int theIndex) const
{
	if (theIndex < 0 || theIndex >= listSize)
	{
		std::ostringstream s;
...
// 函数调用和实现要在一个文件中,只包含"charter5_arrayList.h"会报错
void _chapter5_concrete_class_instantiation()
{
	_chapter5_arrayList<double> arr(20);
	int idx = 0;
	double x = 1.0;
