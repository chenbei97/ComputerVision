...
public:
    ...
    _chapter5_arrayList(int initialCapacity = 10); // 默认构造

    // 时间复杂度为O(n)
    _chapter5_arrayList(const _chapter5_arrayList<T>&); // 复制构造,把 _charter5_arrayList<T>整体看成1个数据类型

    ~_chapter5_arrayList() { delete[] element; }//析构函数
