#include "chapter5_vectorList.h"
#include <numeric>
#include <algorithm>
using namespace std;
void _chapter5_vector_test();
void _chapter5_linearList_by_vector()
{
	// 对容器的性质进行测试,便于对charter5_vectorList.h中vector的使用进行解释
	_chapter5_vector_test();
	
	// 对实现的vectorList进行测试
	_chapter5_linearList<double>* x = new _chapter5_vectorList<double>(20); // 父类指针可以指向子类指针
	// 但是x没有子类的方法,即不存在x->capacity();
	_chapter5_vectorList<int> y(2), z; // 直接创建实例,子类存在capacity()方法
	...
	y.insert(0, 6);
	y.insert(0, 7);
	for (_chapter5_vectorList<int>::iterator i = y.begin(); //iterator使用了别名实际上就是vector自带的
		i != y.end(); i++)
		cout << *i << "  ";
	cout << endl;
