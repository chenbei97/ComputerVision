...
#ifndef chapter5_vectorList_
#define chapter5_vectorList_
#include "chapter5_linearList.h"
#include "chapter1_illegalIndex.h"
...
// statement
template<class T>
class _chapter5_vectorList : public _chapter5_linearList<T>
{
public:
	_chapter5_vectorList(int initialCapacity = 10);
	_chapter5_vectorList(const _chapter5_vectorList<T>& theList);
	~_chapter5_vectorList() { delete element; }
...
// constructed
template<class T>
_chapter5_vectorList<T> ::_chapter5_vectorList(int initialCapacity)
{
...
template<class T>
_chapter5_vectorList<T> ::_chapter5_vectorList(const _chapter5_vectorList<T>& theList)
{
...
template<class T>
void _chapter5_vectorList<T>::checkIndex(int theIndex) const
{
...
template<class T>
T& _chapter5_vectorList<T>::get(int theIndex) const
{
	checkIndex(theIndex);
...
template<class T>
int _chapter5_vectorList<T>::indexOf(const T& theElement) const
{
...
template<class T>
void _chapter5_vectorList<T>::erase(int theIndex)
{
	checkIndex(theIndex);
...
template<class T>
void _chapter5_vectorList<T>::insert(int theIndex, const T& theElement)
{
	if (theIndex < 0 || theIndex > size())
	{// invalid index
		ostringstream s;
...
template<class T>
void _chapter5_vectorList<T>::output(std::ostream& out) const
{
...
// overload <<
template <class T>
std::ostream& operator<<(std::ostream& out, const _chapter5_vectorList<T>& x)
{
	x.output(out); return out;
}
#endif chapter5_vectorList_
