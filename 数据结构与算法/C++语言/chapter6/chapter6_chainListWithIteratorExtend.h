...
#ifndef chapter6_chainListWithIteratorExtend_
#define chapter6_chainListWithIteratorExtend_
#include <iostream>
#include <sstream>
#include <string>
#include "myExceptions.h"
#include "chapter6_chainNode.h"
#include "chapter6_linearListExtend.h" // 继承拓展
#include "chapter6_chainListWithIterator.h" // 借助已经实现的chain修改2个函数和增加成员即可
template<class T>
class _chapter6_chainListWithIteratorExtend : public _chapter6_chainListWithIterator<T>, _chapter6_linearListExtend<T>
{
public:
    // 构造函数不同于普通方法,需要继承_charter6_chainListWithIterator
    // 注意如何继承的形参,前后需要一致
    _chapter6_chainListWithIteratorExtend(int initialCapacity = 10)  : 
        _chapter6_chainListWithIterator<T>(initialCapacity) {} 
    _chapter6_chainListWithIteratorExtend(const _chapter6_chainListWithIteratorExtend<T>& theList) : 
        _chapter6_chainListWithIterator<T>(theList) {}
    ...
    {
        return _chapter6_chainListWithIterator<T>::get(theIndex); // 使用继承调用方法
    }
    int indexOf(const T& theElement) const
    {
        return _chapter6_chainListWithIterator<T>::indexOf(theElement);
    }
    void output(ostream& out) const
    {
        _chapter6_chainListWithIterator<T>::output(out);
    }
    void clear()
    {
        // 和析构函数的代码一样
...
protected:
    _chapter6_chainNode<T>* lastNode;  // 尾节点指针
};

// erase函数重新实现
template<class T>
void _chapter6_chainListWithIteratorExtend<T>::erase(int theIndex)
{
...
// insert函数重新实现
template<class T>
void _chapter6_chainListWithIteratorExtend<T>::insert(int theIndex, const T& theElement)
{
    ...
    { // 如果一开始有头节点,但是插入的节点是在头节点之后,就会有影响
        ...
        for (int i = 0; i < theIndex - 1; i++)
            p = p->next;
        p->next = new _chapter6_chainNode<T>(theElement, p->next);//新创建的index节点的信息在上1个index-1(p)存储，如果是尾插theIndex=listSize，p执行了listSize-1次,p绑定的信息就是listSize位置的节点
...
// push_back的实现
template<class T>
void _chapter6_chainListWithIteratorExtend<T>::push_back(const T& theElement)
{
    // 创建1个指向NULL的新节点的指针
...
#endif chapter6_chainListWithIteratorExtend_
