...
#include "chapter6_studentRecord1.h"
// 排序方法是独立实现的没有作为链表类的成员函数
// 注意使用的是带迭代器的单向链表，不是拓展的单向链表或者单向循环链表
// 使用的学生分数测试场景是_charter6_studentRecord1(只有!=和<<重载)
void _chapter6_studentRecord_binsort(_chapter6_chainListWithIterator<_chapter6_studentRecord1>& theChain, int range);
void _chapter6_StudentRecord1_binSort_testCode()
{
	typedef _chapter6_chainListWithIterator<_chapter6_studentRecord1> chain; // 名字太长取个别名
	typedef  _chapter6_studentRecord1 stu; // 别名只在局部域内有效
	stu s;
	chain c;
	...
	for (int i = 1; i <= 20; i++)  // 给20个学生初始化
	{
		...
		c.insert(0, s); // 把最低分头插,那么最高分排在首位,最低分是尾节点,是最坏的测试数据
	}
	cout << "The unsorted chain is" << endl;
	cout << "  "<<c << endl;
	_chapter6_studentRecord_binsort(c, 10); // 10分是最高档分数
